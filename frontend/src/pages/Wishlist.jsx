...
const Wishlist = () => {
  ...
  return (
    // 1. Added min-height: 100vh so the container stretches to fill the screen
    <div style={{ 
      ...
      {/* 2. Added flex: 1 to this div so it expands to fill empty space */}
      <div style={{ flex: 1 }}>
        {wishlist.length === 0 ? (
          <div style={{
            textAlign: 'center',
              ...
              >
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.65rem' }}>
                  <button 
                    ...
                    style={{ background: 'linear-gradient(135deg, #ec4899 0%, #f43f5e 100%)', color: '#ffffff', border: 'none', padding: '0.8rem', borderRadius: '12px', fontWeight: '800', cursor: 'pointer' }}
                  >
                    Move to Cart Basket
                  </button>
                  <button 
                    onClick={() => toggleWishlist(item)}
                    style={{ background: 'transparent', color: '#94a3b8', border: '1px solid rgba(255,255,255,0.15)', padding: '0.55rem', borderRadius: '12px', cursor: 'pointer' }}
                  >
                    Remove Pin
                  </button>
                </div>
